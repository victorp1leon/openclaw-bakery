...
export type OrderReportPeriod = "today" | "tomorrow" | "week";
...
function buildWeekDateKeys(now: Date, timezone: string): Set<string> {
  ...
  for (let i = -offsetToMonday; i <= 6 - offsetToMonday; i += 1) {
    out.add(toDateKeyFromDate(new Date(now.getTime() + i * DAY_MS), timezone));
  }
...
function matchesPeriod(args: {
  ...
  const todayKey = toDateKeyFromDate(args.now, args.timezone);
  if (args.period === "today") {
    return args.row._dateKey === todayKey;
  }

  if (args.period === "tomorrow") {
    const tomorrowKey = toDateKeyFromDate(new Date(args.now.getTime() + DAY_MS), args.timezone);
    return args.row._dateKey === tomorrowKey;
  }

  const weekKeys = buildWeekDateKeys(args.now, args.timezone);
      ...
      try {
        ...
        if (!result.timedOut && result.exitCode === 0 && !errInfo) {
          ...
          const filtered = parsedRows
            .filter((row) => matchesPeriod({ row, period: args.period, timezone, now: now() }))
            .sort(sortRows)
          ...
          return {
            ...
            orders: filtered,
            detail: `report-orders executed (provider=gws, attempt=${attempt})`
          };
        }
