...
- `schedule.day_view` v1: agenda diaria read-only por fecha (`hoy|mañana|fecha explicita`) sobre `Pedidos` via `gws`, con salida en bloques `deliveries`, `preparation` y `suggestedPurchases`, incluyendo flujo de dato faltante (`schedule_day_query`) sin confirm flow.
...
- Pending: reportes avanzados (reminders), scheduling, customers, and costing/profitability.
