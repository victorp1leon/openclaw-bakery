...
17. `admin.allowlist`: operacion admin de allowlist (`admin allowlist`, `admin allowlist add/remove <chat_id>`) -> parser deterministico runtime -> `view` directo sin confirmacion y `add/remove` con confirm flow -> ejecucion via tool `adminAllowlist` con guardrails (`self-remove` bloqueado + minimo de allowlist) -> respuesta con `Ref/trace_ref` (mutacion controlada en memoria runtime).
18. `schedule.week_view`: consulta de agenda semanal (`agenda de esta semana`, `agenda semanal de 2026-03-23`) -> resolucion de semana lunes-domingo en timezone runtime -> agregacion read-only de 7 ejecuciones `schedule.day_view` -> respuesta consolidada (`days/reminders`, `preparation`, `suggestedPurchases`) con `inconsistencies` por `dateKey` y `Ref/trace_ref` (sin confirmacion ni mutacion).
